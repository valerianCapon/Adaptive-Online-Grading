...
A = np.zeros(shape=(2, nb_colors), dtype=np.double)
b = np.zeros( shape=(2, 1), dtype=np.double)
row_nb = 0
...
#TODO
ssr_check = 0.99
while SSR <= ssr_check-nb_loop/(10*nb_colors+10) and nb_loop < nb_colors*2:
    print("\n============================> LOOP [ ",nb_loop," ] <============================")
    ...
    secu = 0
    limit_broken = secu > (nb_colors*10) 
    already_done = is_assessed(colors_to_asses, assessments)
    ...
    nb_loop += 1
    print("ssr_checker =",ssr_check-nb_loop/(3*nb_colors+10))
